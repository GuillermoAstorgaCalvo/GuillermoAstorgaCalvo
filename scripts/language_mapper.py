...
import os
from error_handling import LanguageMappingError, log_and_raise, get_logger
...
class LanguageMapper:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.logger.info(f"LanguageMapper initialized with {len(self.extension_to_language)} extension mappings and {len(self.filename_to_language)} filename mappings")
            
        except Exception as e:
            log_and_raise(LanguageMappingError, 
    ...
    def map_extension_to_language(self, extension: str) -> str:
        ...
        try:
            ...
            if extension in self.extension_to_language:
                ...
                self.logger.debug(f"Mapped extension '{extension}' to language '{language}'")
                return language
            
            # Check aliases
            for aliased_language, aliases in self.language_aliases.items():
                if extension in aliases:
                    self.logger.debug(f"Mapped extension '{extension}' to language '{aliased_language}'")
    ...
    def map_filename_to_language(self, filename: str) -> str:
        ...
        try:
            ...
            if basename in self.filename_to_language:
                ...
                self.logger.debug(f"Mapped filename '{basename}' to language '{language}'")
                return language
    ...
    def get_supported_languages(self) -> List[str]:
        ...
        try:
            ...
            self.logger.debug(f"Retrieved {len(supported_languages)} supported languages")
    ...
    def get_language_stats(self, extension_stats: Dict[str, Any]) -> Dict[str, Dict[str, int]]:
        ...
        try:
            ...
            self.logger.info(f"Processed {processed_extensions} extensions into {len(language_stats)} languages (excluded {excluded_count} config/non-code files)")
