...
from error_handling import (
    setup_logging, ConfigError, ValidationError, log_and_raise, 
    get_logger, ErrorCodes, with_error_context
)
...
class ConfigManager:
            ...
            if not isinstance(self._config, dict):
                log_and_raise(
                    ConfigError(
                        ...
                        context={'config_path': self.config_path, 'config_type': type(self._config).__name__}
                    ),
                    logger=logger
                )
            
            logger.debug(f"Configuration contains {len(self._config)} top-level sections")
    ...
    def get_repositories(self) -> List[Dict[str, str]]:
        ...
        try:
            ...
            logger.debug(f"Retrieved {len(repositories)} repositories from configuration")
            return repositories
            ...
            log_and_raise(
                ConfigError(
                    ...
                    error_code=ErrorCodes.CONFIG_INVALID,
                    context={'section': 'repositories', 'attribute_error': str(e)}
                ),
                logger=logger
            )
            return []  # This line will never be reached due to log_and_raise
    ...
    def get_github_config(self) -> Dict[str, str]:
        ...
        try:
            ...
            logger.debug(f"Retrieved GitHub configuration with {len(github_config)} settings")
    ...
    def get_processing_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(f"Retrieved processing configuration with {len(processing_config)} settings")
    ...
    def get_report_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(f"Retrieved report configuration with {len(report_config)} settings")
    ...
    def get_report_title(self) -> str:
        """Get report title."""
        try:
            title = self.get_report_config().get('title', 'Repository Statistics Report')
    ...
    def get_date_format(self) -> str:
        ...
        try:
            date_format = self.get_report_config().get('date_format', '%Y-%m-%d %H:%M:%S')
    ...
    def get_artifacts_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(f"Retrieved artifacts configuration with {len(artifacts_config)} settings")
    ...
    def get_stats_filename(self) -> str:
        """Get stats filename."""
        try:
            filename = self.get_artifacts_config().get('stats_filename', 'repo_stats.json')
                        ...
                        if not isinstance(repo, dict):
                            ...
                            for field in required_fields:
                                if field not in repo:
                                    errors.append(f"Repository {i} missing required field: {field}")
                ...
                if not isinstance(patterns, dict):
                    ...
                    for pattern_type in required_patterns:
                        if pattern_type not in patterns:
                            errors.append(f"Missing author pattern type: {pattern_type}")
                        elif not isinstance(patterns[pattern_type], list):
                            errors.append(f"Author pattern {pattern_type} must be a list")
