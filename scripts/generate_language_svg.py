...
from error_handling import SvgGenerationError, log_and_raise, get_logger
...
def generate_language_svg_bar_chart(language_stats: dict, output_path: str):
    ...
    try:
        ...
        logger.info(f"Generating SVG for {len(sorted_langs)} languages with {total_loc:,} total LOC")
        ...
        dwg.save()
        logger.info(f"Successfully generated SVG with {processed_languages} languages at {output_path}")
        ...
        log_and_raise(SvgGenerationError, 
                     f"Error generating SVG chart: {e}", 
                     error_code="SVG_GENERATION_ERROR")


def load_unified_stats(unified_stats_path: str = "unified_stats.json") -> Dict[str, Any]:
        ...
        for lang, stats in language_stats.items():
            try:
                if isinstance(stats, dict) and 'loc' in stats:
                    loc = stats.get('loc', 0)
                    if loc > 0 and lang.lower() not in ['other', 'unknown', 'text', 'markdown']:
...
if __name__ == "__main__":
    try:
        ...
        get_logger(__name__).error(f"Unexpected error in SVG generation: {e}", exc_info=True)
        sys.exit(1) 