...
def run_cloc_on_repo(repo_path: Path) -> dict:
    ...
    try:
        ...
        # nosec B603: cloc is a trusted tool for code analysis, args are hardcoded
        # nosec B607: cloc is a standard tool installed via package manager
        result = subprocess.run(  # nosec B603 B607
            [
                "cloc",
                "--json",
                "--quiet",
                f"--timeout={CLOC_PROCESS_TIMEOUT_SECONDS}",
        ...
        try:
            ...
            if isinstance(data, dict):
                ...
                logger.error("cloc output is not a dictionary")
                return {}
        ...
        logger.error(
            f"cloc execution timed out after {CLOC_TIMEOUT_SECONDS} seconds: {e}"
        )
        return {}
...
def generate_svg_content(top_languages: list[tuple[str, int]], total_loc: int) -> str:
    ...
    for lang, loc in top_languages:
        ...
        svg.append(
            f'<rect x="150" y="{y}" width="{bar_width}" height="{bar_height - 8}" fill="#4F8EF7" />'
        )
        svg.append(
            f'<text x="10" y="{y + bar_height // 2}" font-size="14" fill="#222">{lang}</text>'
        )
        svg.append(
            f'<text x="{150 + bar_width + 10}" y="{y + bar_height // 2}" font-size="14" fill="#222">{loc:,}</text>'
        )
        y += bar_height
    svg.append("</svg>")
    return "\n".join(svg)
...
def generate_language_svg_bar_chart(
    language_stats: dict[str, Any], output_path: str
) -> None:
...
def main() -> None:
    language_stats: dict[str, Any] = {}  # Ensure language_stats is always defined
    try:
        ...
        try:
            ...
            logger.warning(
                f"cloc analysis failed: {e}", extra={"repo_path": str(repo_path)}
            )
            cloc_language_stats = {}
        git_fame_data = git_fame_parser.execute_git_fame(
            str(repo_path), config.get_git_fame_format()
        )
        ...
        git_fame_bytype_data = git_fame_parser.execute_git_fame(
            str(repo_path), config.get_git_fame_format(), by_type=True
        )
        language_stats = {}
        if git_fame_bytype_data:
            extension_stats = git_fame_parser.extract_extension_stats(
                git_fame_bytype_data
            )
