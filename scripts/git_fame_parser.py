...
class GitFameParser:
    ...
    def _check_git_fame_version(self) -> bool:
        ...
        try:
            result = subprocess.run(['git-fame', '--version'], 
                                  capture_output=True, text=True, timeout=10)
            if result.returncode == 0:
                version = result.stdout.strip()
                logger.info(f"Git fame version: {version}")
                logger.debug(f"Git fame version check completed")
                return True
            else:
    ...
    def execute_git_fame(self, repo_path: str, output_format: str = "json", by_type: bool = False) -> Optional[Dict[str, Any]]:
        ...
        if not isinstance(output_format, str) or output_format not in ['json', 'csv', 'tsv']:
            logger.error(f"Invalid output format: {output_format}. Must be one of: json, csv, tsv")
            return None
        ...
        try:
            ...
            try:
                ...
                if result.returncode != 0:
                    logger.error(f"Git fame execution failed with return code {result.returncode}")
                ...
                logger.error(f"Git fame execution timed out after {self.timeout_seconds} seconds: {e}")
                return None
            ...
            logger.error(f"Unexpected error during git fame execution: {e}", 
    ...
    def parse_author_data(self, author_data: Any) -> Optional[Tuple[str, int, int, int]]:
        ...
        try:
            ...
            logger.warning(f"Failed to parse author data: {e}", extra={'author_data': str(author_data)})
            return None
    ...
    def extract_extension_stats(self, data: Dict[str, Any]) -> Dict[str, Dict[str, Union[int, float]]]:
        """
