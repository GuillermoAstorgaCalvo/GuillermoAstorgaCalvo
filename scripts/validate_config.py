...
def validate_repository_config(repo: Dict[str, Any], index: int) -> List[str]:
    ...
    if token_type and token_type not in valid_token_types:
        errors.append(f"Repository '{repo_name}' has invalid token_type: {token_type}. Must be one of: {valid_token_types}")
...
def validate_config_file(config_path: str) -> List[str]:
    ...
    try:
        ...
        duplicate_names = [name for name in set(repo_names) if repo_names.count(name) > 1]
        if duplicate_names:
            errors.append(f"Duplicate repository names found: {duplicate_names}")
        
        # Check for duplicate artifact names
        artifact_names = [repo.get('artifact_name') for repo in repositories if repo.get('artifact_name')]
        duplicate_artifacts = [name for name in set(artifact_names) if artifact_names.count(name) > 1]
...
def print_validation_summary(repositories: List[Dict[str, Any]], errors: List[str]):
...
def main():
    ...
    if not is_valid:
        ...
        print("  - Ensure all repositories have required fields: name, display_name, branch, artifact_name, organization, token_type")
