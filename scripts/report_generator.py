...
from error_handling import DataProcessingError, log_and_raise, get_logger
...
class MarkdownReportGenerator:
        ...
        try:
            ...
            if stats.guillermo_unified:
                ...
                loc_pct, commits_pct, files_pct = self.calculate_distribution_percentages(
                    guillermo_stats, 
                    AuthorStats(loc=stats.total_loc, commits=stats.total_commits, files=stats.total_files)
                )
                
                summary += f"**📈 Contribution Distribution:**\n"
    ...
    def generate_report(self, stats: UnifiedStats, config: Optional[Dict[str, Any]] = None) -> str:
        report = ""
        try:
    ...
    def save_report(self, stats: UnifiedStats, filename: str = "STATS.md", config: Optional[Dict[str, Any]] = None) -> None:
        """
...
class JSONReportGenerator:
    ...
    def _analyze_tech_stack(self, language_stats: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.warning("DependencyAnalyzer not available, using fallback tech stack analysis")
        ...
        try:
            ...
            for category in categories.values():
                ...
                if isinstance(techs, list):
                    category['technologies'] = sorted(list(set(techs)))
            return categories
        except Exception as e:
            log_and_raise(DataProcessingError(
    ...
    def _analyze_complexity(self, stats: UnifiedStats) -> Dict[str, Any]:
        try:
            ...
            log_and_raise(DataProcessingError(
                f"Failed to analyze complexity: {e}",
                context={'stats': str(stats)}
            ), logger=self.logger)
            return {}  # This line will never be reached due to log_and_raise
    
    def _generate_insights(self, stats: UnifiedStats, language_analysis: Dict[str, Any], repo_analysis: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            log_and_raise(DataProcessingError(
                ...
                context={'stats': str(stats), 'language_analysis': str(language_analysis), 'repo_analysis': str(repo_analysis)}
            ), logger=self.logger)
            return {}  # This line will never be reached due to log_and_raise
