...
class AnalyticsReporter:
        ...
        for period, metrics in velocity_data.items():
            ...
            for metric in metrics[-5:]:  # Show last 5 periods
                start_date = datetime.fromisoformat(
                    metric.start_date.replace("Z", "+00:00")
                ).replace(tzinfo=None)
                end_date = datetime.fromisoformat(
                    metric.end_date.replace("Z", "+00:00")
                ).replace(tzinfo=None)
                period_str = (
        ...
        if top_languages:
            ...
            if len(top_languages) >= 3:
                ...
                for i, lang in enumerate(top3, 1):
                    markdown += (
                        f"- {i}. {lang['language']} ({lang['percentage']:.1f}%)\n"
                    )
                markdown += "\n"
        ...
        if "monthly" in trends and "error" not in trends["monthly"]:
            ...
            if "total_loc" in monthly:
                ...
                markdown += self._create_simple_chart(
                    loc_data["start"], loc_data["end"], "LOC"
                )
                markdown += "\n"
            ...
            if "total_commits" in monthly:
                ...
                markdown += self._create_simple_chart(
                    commits_data["start"], commits_data["end"], "Commits"
                )
                markdown += "\n"
    ...
    def generate_insights_section(
        self, trends: dict[str, Any], language_analysis: dict[str, Any]
    ) -> str:
        """
        ...
        if "monthly" in trends and "error" not in trends["monthly"]:
            ...
            if "total_loc" in monthly:
                ...
                if loc_growth > 20:
                    insights.append(
                        "🚀 **Strong code growth detected** - Excellent development velocity!"
                    )
                elif loc_growth > 10:
                    insights.append(
                        "📈 **Good code growth** - Maintaining steady development pace."
                    )
                elif loc_growth < 0:
                    insights.append(
                        "⚠️ **Code reduction detected** - Consider if this is intentional refactoring."
                    )
            ...
            if "total_commits" in monthly:
                ...
                if commits_growth > 30:
                    insights.append(
                        "⚡ **High commit frequency** - Great development activity!"
                    )
                elif commits_growth < 5:
                    insights.append(
                        "📝 **Low commit activity** - Consider more frequent commits."
                    )
        ...
        if language_analysis:
            ...
            if total_languages > 10:
                insights.append(
                    f"🌍 **Diverse tech stack** - {total_languages} languages detected!"
                )
            elif total_languages > 5:
                insights.append(
                    f"🔄 **Multi-language project** - {total_languages} languages in use"
                )
            ...
            if top_languages:
                ...
                if primary_percentage > 70:
                    insights.append(
                        f"🎯 **{primary_lang['language']} focused** - {primary_percentage:.1f}% of codebase"
                    )
                elif primary_percentage < 30:
                    insights.append(
                        "🔄 **Balanced codebase** - Good language distribution"
                    )
            ...
            if "weekly" in velocity and velocity["weekly"]:
                ...
                if recent_velocity:
                    if recent_velocity.loc_velocity > 100:
                        insights.append(
                            "⚡ **High development velocity** - Excellent productivity!"
                        )
                    elif recent_velocity.loc_velocity < 10:
                        insights.append(
                            "🐌 **Low development velocity** - Consider increasing activity."
                        )
...
def generate_analytics_report(data: dict[str, Any]) -> str:
    ...
    try:
        ...
        if unified_stats:
            ...
            report += (
                f"- **Total Lines of Code:** {unified_stats.get('total_loc', 0):,}\n"
            )
            report += (
                f"- **Total Commits:** {unified_stats.get('total_commits', 0):,}\n"
            )
            ...
            if len(analytics_history) >= 2:
                ...
                try:
                    first_date = datetime.fromisoformat(
                        first_point.get("timestamp", "").replace("Z", "+00:00")
                    )
                    last_date = datetime.fromisoformat(
                        last_point.get("timestamp", "").replace("Z", "+00:00")
                    )
